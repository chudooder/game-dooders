...
public class Merc extends Entity implements Collideable {
	...
	public void beginStep() {
		...
		if (inputs.get(Input.MOUSE) != null) {
			...
			angle = (float) Math.toDegrees(Math.atan2(loc[1], loc[0]));
			/*
			 * Why is this necessary? if (mY < 0) angle += 360;
			 */
		}
	...
	private double raytrace(ArrayList<Wall> walls, Wall ignore, double rayAngle) {
		...
		for (Wall w : walls) {
			...
			if (w.equals(ignore))
				continue;
			float check1 = (float) Math.toDegrees(Math.atan2(w.y - centerY, w.x
					- centerX));
			float check2 = (float) Math
					.toDegrees(Math.atan2(w.y + w.getHeight() - centerY, w.x
							+ w.getWidth() - centerX));
	...
	private void renderLightMap() {
		...
		// Get a list of angles to raytrace to, then raytrace and get the actual
		// points
		for (Wall wall : walls) {
			for (int i = wall.x; i <= wall.getEndX(); i += wall.getWidth()) {
				for (int j = wall.y; j <= wall.getEndY(); j += wall.getHeight()) {
					double r = Math.toDegrees(Math
							.atan2(j - y - 16, i - x - 16));
					boolean ignore = false;
					if (i == wall.x && j == wall.y || i == wall.getEndX()
							&& j == wall.getEndY()) {
